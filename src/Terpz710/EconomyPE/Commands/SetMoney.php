                ...
                if (Money::existPlayer($name)) {
                    if (isset($args[1])) {
                        if (is_numeric($args[1])) {
                            Money::setMoney($name, intval($args[1])); // Cast to int
                            $player->sendMessage(Money::getConfigReplace("setmoney_msg", [strtolower("{money}"), strtolower("{player}")], [intval($args[1]), $name])); // Cast to int
